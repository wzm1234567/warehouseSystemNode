 ...
 * @param {String} req.body.name 物料名称的id
 * @param {String} req.body.pinpai 物料品牌id
 * @param {String} req.body.num 总重量 （吨）
 * @param {String} req.body.guige 物料规格id
 * @param {String} req.body.price 入库金额
 * @param {String} req.body.card 运输车辆
 * @param {String} req.body.time 入库时间 时间戳
 * @param {String} req.body.is_stash_id 仓库id
 * @param {String} req.body.num_flag 1:吨 2:千克 3:斤
 * @param {String} req.body.price_option 1:元 2:万元
 */
router.post("/addwarehousing", async (req, res) => {
  try {
    ...
    const queries = [
      {
        sql: 'insert into warehousing(name,pinpai,num,specifications,number,price,card,time,stash_label) values(?,?,?,?,?,?,?,?,?)',
        params: [req.body.name, req.body.pinpai, req.body.num, req.body.guige, req.body.number, req.body.price, req.body.card, req.body.time, dataStash[0].name]
      },
      {
        sql: 'insert into stash_info(is_stash_id,name,pinpai,type,weight,price,guige,num,time) values(?,?,?,?,?,?,?,?,?)',
        params: [req.body.is_stash_id, req.body.name, req.body.pinpai, 0, req.body.num, req.body.price, req.body.guige, req.body.number, req.body.time]
      },
      {
        sql: 'update material_guige set surplus=? where id=?',
        params: [Number(data3[0].surplus) + Number(req.body.number), data3[0].id]
      }
    ]
    const result = await db.executeTransaction(queries)
...
router.get("/deletewarehousing", async (req, res) => {
  try {
    ...
    await db.executeTransaction(querys)
